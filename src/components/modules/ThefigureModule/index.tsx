...
export const ThefigureModule: React.FC = () => {
  ...
  const [sorting, setSorting] = useState("A-Z");
    ...
    const handleSortingZA = () => {
      setSorting("Z-A");
      let temp = dataAngkatanSearched
      temp.sort((a, b) => a["Nama Lengkap"].localeCompare(b["Nama Lengkap"])).reverse()
      setDataAngkatanSearched(temp)
    }
    ...
    const handleSearch = (e) => {
      ...
      let value = (document.getElementById("searchBar") as HTMLInputElement).value;
      if(value=== "") {
        setDataAngkatanSearched(dataAngkatan)
      } else {
        let temp: object[] = []
        dataAngkatan.map((key) => {
          if(key['Nama Lengkap'].toLowerCase().includes(value.toLowerCase()) || key['Nama Panggilan'].toLowerCase().includes(value.toLowerCase())) {
            temp.push(key)
          }
        })
        setDataAngkatanSearched(temp)
        ...
        <div className=''>
          <div className="items-center px-4 flex justify-center" >
            <div className="relative w-[80vw] sm:max-w-2xl sm:mx-auto">
              ...
              <form onSubmit={handleSearch} role="form" className="relative flex z-50 bg-white rounded-full">
                  <input onKeyUp={handleSearch} id='searchBar' onKeyDown={() =>{}} type="text" placeholder="Search your friend here..." className="rounded-full flex-1 pl-6 py-4 text-gray-700 focus:outline-none" />
                  <button onClick={handleSearch} className="bg-[#0E1F58] text-white rounded-full font-semibold px-7 py-4 hover:bg-indigo-800">Search</button>
                </form>
              </div>
            </div>
          </div>
...
function AZActiveIcon(props) {
    return (
    <svg  {...props} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="w-6 h-6">
